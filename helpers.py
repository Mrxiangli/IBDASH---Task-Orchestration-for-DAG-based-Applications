...
# build the dictionary for cost
def build_cost_dictionary(ED_cost):
    cost = {}
    for each in ED_cost:
        cost[int(each[0])]=each[1]
    return cost
...
def send_ntwk_test(s,transmission_err_prov):
    LABEL_SIZE = 10
    s.send("P".encode())
    s.send(str(transmission_err_prov).ljust(LABEL_SIZE).encode())
...
def network_test():
    ...
    for idx,ip_address in enumerate(global_var.device_list):
        ...
        if result.avg_rtt != 0:
            p2p_test[idx]=round(((1024/(result.avg_rtt/2))*1000)/1000000,3)
    return p2p_test
