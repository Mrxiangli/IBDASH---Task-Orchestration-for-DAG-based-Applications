...
def mapping(file, count):
    ...
    for idx,row in df.iterrows():
        if row['Country/Region'] not in result.keys():
            ...
            result[row['Country/Region']]["Deaths"]+=row['Deaths']
            result[row['Country/Region']]["Recovered"]+=row['Recovered']
    with open(f"split_output_1_{count}.json","w") as outfile:
    #with open(f"split_output_1_{count}_{rd.randint(1,10000)}.json","w") as outfile:
...
if __name__ =='__main__':
    ...
    file=f'split_input_1_{args.count}.csv'
   # file=f'split_input_1_0.csv'

    start=timer.time()
    mapping(file, args.count)
    #mapping(file, 0)
    end=timer.time()
    print("map1: "+str((end-start)))
